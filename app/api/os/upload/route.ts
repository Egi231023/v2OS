...
export async function POST(req:NextRequest){try{const {subject,state,root}=await readRoot(),actor=resolveActor(state,root,req.cookies.get('os_demo_actor')?.value);if(!actor)throw new DomainError('Access unavailable.',403);
 ...
 const result=await gateway({action:'upload',subject,actor:actor.id,path,mime:file.type,base64:btoa(binary)});
